...
def update_conv_hlop(x: torch.Tensor, hlop_module, iteration=5, fix_subspace_id_list: Optional[List[int]] = None,
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride, snn_setting, neuron_type='lif', ss=False, hlop_with_wfr=True,
                 hlop_spiking=False, hlop_spiking_scale=20., hlop_spiking_timesteps=1000., proj_type='input'):
        ...
        if self.ss:
            self.conv1 = SSConv2dProj(in_channels, out_channels, kernel_size=3, stride=stride, padding=1, bias=False,
                                      proj_type=proj_type)
        else:
            self.conv1 = Conv2dProj(in_channels, out_channels, kernel_size=3, stride=stride, padding=1, bias=False,
                                    proj_type=proj_type)
        hlop_modules.append(HLOP(in_channels * 3 * 3, spiking=self.hlop_spiking, spiking_scale=self.hlop_spiking_scale,
                                 spiking_timesteps=self.hlop_spiking_timesteps))
        self.bn2 = nn.BatchNorm2d(out_channels)

        if self.ss:
            self.conv2 = SSConv2dProj(out_channels, self.expansion * out_channels, kernel_size=3, stride=1, padding=1,
                                      bias=False, proj_type=proj_type)
        else:
            self.conv2 = Conv2dProj(out_channels, self.expansion * out_channels, kernel_size=3, stride=1, padding=1,
                                    bias=False, proj_type=proj_type)
        hlop_modules.append(HLOP(out_channels * 3 * 3, spiking=self.hlop_spiking, spiking_scale=self.hlop_spiking_scale,
                                 spiking_timesteps=self.hlop_spiking_timesteps))

        self.stride = stride
        if stride != 1 or in_channels != self.expansion * out_channels:
            if self.ss:
                self.downsample = SSConv2dProj(in_channels, self.expansion * out_channels, kernel_size=1, stride=stride,
                                               padding=0, proj_type=proj_type)
            else:
                self.downsample = Conv2dProj(in_channels, self.expansion * out_channels, kernel_size=1, stride=stride,
                                             padding=0, proj_type=proj_type)
            hlop_modules.append(
                HLOP(in_channels * 1 * 1, spiking=self.hlop_spiking, spiking_scale=self.hlop_spiking_scale,
                     spiking_timesteps=self.hlop_spiking_timesteps))
        else:
    ...
    def forward(self, x, task_id=None, projection=False, proj_id_list=[0], update_hlop=False, fix_subspace_id_list=None,
        ...
        if self.downsample is None:
            ...
            if update_hlop:
                if self.hlop_with_wfr:
                    ...
                    out_ = out
                update_conv_hlop(out_, self.hlop_modules[2], hlop_iteration, fix_subspace_id_list, kernel_size=1,
        ...
        if update_hlop:
            ...
            update_conv_hlop(out, self.hlop_modules[0], hlop_iteration, fix_subspace_id_list, kernel_size=3, padding=1,
                             stride=1)
        out = out_
        ...
        if update_hlop:
            if self.hlop_with_wfr:
                # update hlop by weighted firing rate
                out = weight_rate_spikes(out, self.timesteps, self.tau, self.delta_t)
            update_conv_hlop(out, self.hlop_modules[1], hlop_iteration, fix_subspace_id_list, kernel_size=3, padding=1,
                             stride=1)
        out = out_
...
class SpikingResNet(nn.Module):
    ...
    def __init__(self, block, num_blocks, snn_setting, neuron_type='lif', nf=64, num_classes=10, share_classifier=False,
                 ss=False, hlop_with_wfr=True, hlop_spiking=False, hlop_spiking_scale=20., hlop_spiking_timesteps=1000.,
                 proj_type='input', first_conv_stride2=False):
        ...
        if first_conv_stride2:
            ...
            self.hlop_modules.append(HLOP(3 * 5 * 5, spiking=self.hlop_spiking, spiking_scale=self.hlop_spiking_scale,
                                          spiking_timesteps=self.hlop_spiking_timesteps))
        else:
            if self.ss:
            ...
            self.hlop_modules.append(HLOP(3 * 3 * 3, spiking=self.hlop_spiking, spiking_scale=self.hlop_spiking_scale,
                                          spiking_timesteps=self.hlop_spiking_timesteps))
        self.layers = []
        ...
        if share_classifier:
            self.hlop_modules.append(HLOP(fc_size, spiking=self.hlop_spiking, spiking_scale=self.hlop_spiking_scale,
                                          spiking_timesteps=self.hlop_spiking_timesteps))
            if self.ss:
    ...
    def _make_layer(self, block, planes, num_blocks, stride, snn_setting):
        ...
        for stride in strides:
            layers.append(
                block(self.in_planes, planes, stride, snn_setting, self.neuron_type, self.ss, self.hlop_with_wfr,
                      self.hlop_spiking, self.hlop_spiking_scale, self.hlop_spiking_timesteps, self.proj_type))
    ...
    def forward(self, x, task_id=None, projection=False, proj_id_list=[0], update_hlop=False, fix_subspace_id_list=None,
        ...
        if update_hlop:
            ...
            if self.first_conv_stride2:
                update_conv_hlop(x, self.hlop_modules[0], hlop_iteration, fix_subspace_id_list, kernel_size=5,
                                 padding=2, stride=2)
            else:
                update_conv_hlop(x, self.hlop_modules[0], hlop_iteration, fix_subspace_id_list, kernel_size=3,
                                 padding=1, stride=1)
